...
```{r}
set.seed(640)
# sample a smaller subset of data due to large size of df 
dataPlot <- allSubDat %>% sample_n(15000)
```

# histogram of pupil response 
```{r}
pupilRespFig <- ggplot(allSubDat, aes(x = pupil)) +
...
```{r}
ggplot(allSubDat, aes(x = condOrder, y = pupil)) +
...
ggplot(plot_data, aes(x = time, y = pupil, color = condOrder)) +
...
ggplot(plot_data, aes(sample = pupil)) +
  stat_qq() +
  stat_qq_line() +
