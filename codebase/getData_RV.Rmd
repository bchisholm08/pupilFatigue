...
getRData <- function(data) {
  ...
  if (data == 1 ){
    ...
    # Miscellaneous directory for other experiment stuff 
    miscDir <- "M:/Lab_Shared/Brady_C/Projects/pupilEEG_fatigue/statistics/scripts/misc"
    
    # Directory to save extra downsampled data 
    ...
    # Import the new resample function
    ...
    source("resampleSignal.R")  # Now the resample_signal function is available
    ...
    # Prepare bookkeeping table for eye data used 
    ...
    for (subj in subjIDS) {
      ...
      # Read .mat data file (this can be time heavy)
      ...
      # Check existence of "allDat" in .mat file.
      if (!"allDat" %in% names(mat_data)) {
        warning(subj, ": 'allDat' not found in the .mat file.")
        stop()
      }
      
      # Extract "allDat" (expected dimensions: [200 x 2 x 6000])
